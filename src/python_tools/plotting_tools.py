...
def plot_orbits(rs, args, vectors=[]):
    ...
    for r in rs:
        ...
        ax.plot([_r[0, 0]], [_r[0, 1]], [_r[0, 2]], "o", color=_args["colors"][n])
    ...
    if _args["cb_SOI"] is not None:
        ...
        ax.plot_wireframe(
            _x, _y, _z, color=_args["cb_SOI_color"], alpha=_args["cb_SOI_alpha"]
        )
...
def plot_states(ets, states, args):
    ...
    """ Positions """
    ax0.plot(ts, states[:, 0], "r", label=r"$r_x$", linewidth=_args["lw"])
    ax0.plot(ts, states[:, 1], "g", label=r"$r_y$", linewidth=_args["lw"])
    ax0.plot(ts, states[:, 2], "b", label=r"$r_z$", linewidth=_args["lw"])
    ax0.plot(ts, rnorms, "m", label=r"$Norms$", linewidth=_args["lw"])
    ...
    """ Velocities """
    ax1.plot(ts, states[:, 3], "r", label=r"$r_x$", linewidth=_args["lw"])
    ax1.plot(ts, states[:, 4], "g", label=r"$r_y$", linewidth=_args["lw"])
    ax1.plot(ts, states[:, 5], "b", label=r"$r_z$", linewidth=_args["lw"])
    ax1.plot(ts, vnorms, "m", label=r"$Norms$", linewidth=_args["lw"])
...
def plot_velocities(ets, vs, args):
    ...
    ax0.plot(_ets, vs[:, 0], "r", label=r"$v_x$", linewidth=_args["lw"])
    ax0.plot(_ets, vs[:, 1], "g", label=r"$v_y$", linewidth=_args["lw"])
    ax0.plot(_ets, vs[:, 2], "b", label=r"$v_z$", linewidth=_args["lw"])
    ax0.plot(_ets, vnorms, "m", label=r"$Norms$", linewidth=_args["lw"])
    ...
    if _args["legend"]:
        ax0.legend(
            fontsize=_args["legend_fontsize"],
            loc="upper right",
            framealpha=_args["legend_framealpha"],
        )
...
def plot_coes(ets, coes, args):
    ...
    n = 0
    for coe in coes:
        ax0.plot(ts, coe[:, 3], _args["color"], label=_args["labels"][n])
        n += 1
    ...
    n = 0
    for coe in coes:
        ax3.plot(ts, coe[:, 0], _args["color"], label=_args["labels"][n])
        n += 1
    ...
    n = 0
    for coe in coes:
        ax1.plot(ts, coe[:, 1], _args["color"], label=_args["labels"][n])
        n += 1
    ...
    n = 0
    for coe in coes:
        ax4.plot(ts, coe[:, 2], _args["color"], label=_args["labels"][n])
        n += 1
    ...
    n = 0
    for coe in coes:
        ax2.plot(ts, coe[:, 4], _args["color"], label=_args["labels"][n])
        n += 1
    ...
    n = 0
    for coe in coes:
        ax5.plot(ts, coe[:, 5], _args["color"], label=_args["labels"][n])
        n += 1
...
def plot_eclipse_array(ets, arr, args):
    ...
    ax0.plot(_ets, arr, "-o", color=_args["color"], linewidth=_args["lw"], ms=5)

    ax0.grid(linestyle="dotted")
    ax0.set_xlim(_args["xlim"])
    ax0.set_ylim([-1.5, 2.5])
    ax0.set_xlabel(_args["xlabel"], size=_args["labelsize"])
    ax0.set_ylabel(r"$1=Penumbra$, $2=Umbra$", size=_args["labelsize"])
...
def plot_cr3bp_2d(mu, rs, args):
    ...
    n = 0
    for r in rs:
        plt.plot(r[:, 0], r[:, 1], _args["colors"][n], label=_args["labels"][n])
        plt.plot(r[0, 0], r[0, 1], "o", color=_args["colors"][n])
        n += 1
...
def plot_cr3bp_3d(mu, rs, args, vectors=[]):
    ...
    for r in rs:
        ...
        ax.plot([r[0, 0]], [r[0, 1]], [r[0, 2]], "o", color=_args["colors"][n])

        max_val = max([r.max(), max_val])
        n += 1

    ax.plot([-mu], [0], [0], "o", ms=_args["b1_ms"], color=_args["b1_color"])
    ax.plot([1 - mu], [0], [0], "o", ms=_args["b2_ms"], color=_args["b2_color"])
...
def plot_pseudopotential_contours(system, args):
    ...
    if _args["LPs"]:
        ...
        plt.plot(0.5 - system["mu"], 3**0.5 / 2.0, "co", ms=7, label="L4")
        plt.plot(0.5 - system["mu"], -(3**0.5) / 2.0, "mo", ms=7, label="L5")
    ...
    for nx in range(n_vals):
        for ny in range(n_vals):
            omegas[ny, nx] = cr3bp_pseudopotential(system["mu"], [x[nx], y[ny]])
